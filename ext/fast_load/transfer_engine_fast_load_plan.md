# vLLM Fast Load: transfer_engine 首版改造执行计划
...
- 在多卡多进程场景下，元信息文件按 rank 维度切分，避免不同 worker 互相覆盖。
...
### 3) 文件型元信息实现（首版）
- 文件：`vllm/model_executor/model_loader/fast_load/file_meta_provider.py`
- 职责：
  - 从 `VLLM_FAST_LOAD_META_FILE` 读写 JSON 元信息
  - 按 rank 自动生成文件后缀（如 `.rank3.json` 或 `.ranktp0_pp0_ep0.json`）
...
### 4) fast load 编排层
- 文件：`vllm/model_executor/model_loader/fast_load/fast_load.py`
- 职责：
...
## 环境变量设计（首版）
- `VLLM_FAST_LOAD_ENABLE`：是否开启 fast load（默认关）
- `VLLM_FAST_LOAD_META_FILE`：元信息文件基础路径（将自动附加 rank 后缀）
- `VLLM_FAST_LOAD_TE_HOSTNAME`：本进程 transfer_engine 初始化地址（`host:port`），未设置则自动推导 IP + 随机端口
- `VLLM_FAST_LOAD_TE_RPC_THREADS`：可选，默认 2
...
## 交付物
- 代码：`vllm/model_executor/model_loader/fast_load/*`
- 接入修改：`vllm/model_executor/model_loader/base_loader.py`
- 文档：`ext/fast_load/transfer_engine_fast_load_plan.md`、`ext/fast_load/README.md`
